...
var name = person.name,
  age = person.age;
console.log("Destructured name: ".concat(name, ", age: ").concat(age));
"use strict";

document.addEventListener('DOMContentLoaded', function () {
  var container = document.querySelector('.new-artifacts__images');
  var inner = document.querySelector('.new-artifacts__images-inner');
...
var Swiper = function () {
  ...
  function elementIsChildOf(el, parent) {
    ...
    if (!isChild && parent instanceof HTMLSlotElement) {
      var children = _toConsumableArray(element.assignedElements());
    ...
    if (swiper.params.createElements) {
      Object.keys(checkProps).forEach(function (key) {
        if (!params[key] && params.auto === true) {
          var _element = elementChildren(swiper.el, ".".concat(checkProps[key]))[0];
          if (!_element) {
            _element = createElement('div', checkProps[key]);
            _element.className = checkProps[key];
            swiper.el.append(_element);
          }
          params[key] = _element;
          originalParams[key] = _element;
        }
      });
    }
    return params;
...
var swiper = new Swiper('.swiper-container', {
  ...
  effect: 'slide',
  // Эффект перехода ('slide' по умолчанию)
  grabCursor: true,
  // Изменить курсор при перетаскивании
  simulateTouch: true,
  // Разрешить взаимодействие с сенсорными экранами
  touchRatio: 1,
  // Коэффициент чувствительности к касанию
  touchAngle: 45,
  allowTouchMove: true,
  // Разрешить перетаскивание слайдов курсором
  mousewheel: {
    enabled: true // Включить управление слайдером с помощью колеса мыши
  },
  // Брейкпойнты по аналогии с Bootstrap
  breakpoints: {
    320: {
      // Для экранов от 320px и выше
      slidesPerView: 1,
      spaceBetween: 0
    },
    576: {
      // Эквивалентно Bootstrap sm
      slidesPerView: 2,
      spaceBetween: 20
    },
    768: {
      // Эквивалентно Bootstrap md
      slidesPerView: 2,
      spaceBetween: 40
    },
    992: {
      // Эквивалентно Bootstrap lg
      slidesPerView: 3,
      spaceBetween: 20
    },
    1200: {
      // Эквивалентно Bootstrap xl
      slidesPerView: 4,
      spaceBetween: 30
    }
  },
  ...
  navigation: {
    nextEl: '.next-class',
    // должен быть .next-class
    prevEl: '.prev-class' // должен быть .prev-class
  },
